...
namespace Hacks
{
	...
	/////	PvP Disable Checks
	DWORD dwPVP1 = 0x00BC6EE4; //GMS: 00D33CA4: // Skip Check // 0F 84 ?? ?? ?? ?? 2B AE ?? ?? ?? ?? 0F 88 ?? ?? ?? ?? 8B 86 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 83 C4 ?? 3B 86 ?? ?? ?? ?? 0F 85 ?? ?? ?? ?? 8B 96
	BYTE bPVP1[] = {0x90, 0x90, 0x90, 0x90, 0x90, 0x90};
	DWORD dwPVP2 = 0x00BC6F0B; //GMS: 00D33CCB: // Bypass Weapon Check // 0F 85 ?? ?? ?? ?? 8B 96 ?? ?? ?? ?? 8B 44 24 ?? 6A ?? 6A ?? 6A ?? 8D 4C 24 ?? 51 8B 0D ?? ?? ?? ?? 52 50 C7 44 ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B F8 85 FF 0F 8E
	BYTE bPVP2[] = {0x90, 0x90, 0x90, 0x90, 0x90, 0x90};
	DWORD dwPVP3 = 0x00BC6FAC; //GMS: 00D33D6C: // Switch Jump // 77 ?? 0F B6 88 ?? ?? ?? ?? FF 24 8D ?? ?? ?? ?? 8B 54 24 ?? 6A ?? 6A ?? 6A ?? 6A ?? 6A ?? 6A ?? 6A ?? 6A ?? 6A ?? 6A ?? 57 52 8B CE E8 ?? ?? ?? ?? 5F
